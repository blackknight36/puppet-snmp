...
describe 'snmp', :type => 'class' do
  ...
  freebsdish.each do |os|
    describe "for osfamily FreeBSD, operatingsystem #{os}" do
      ...
      it { should contain_package('net-snmp').with(
        :ensure => 'installed',
      )}
      ...
      it { should contain_file('/usr/local/etc/snmp/snmpd.conf').with(
        :ensure  => 'installed',
        :mode    => '0644',
        :owner   => 'root',
        :group   => 'wheel',
      ...
      it { should contain_service('snmpd').with(
        ...
        :hasstatus  => true,
        :hasrestart => true,
        #:require    => [ 'Package[net-snmp]', 'File[var-net-snmp]', ]
      )}
  ...
  context 'on a supported osfamily (RedHat), custom parameters' do
    ...
    describe 'autoupgrade => true' do
      ...
      it { should_not contain_class('snmp::client') }
      it { should contain_file('var-net-snmp').with_ensure('directory') }
      it { should contain_file('snmpd.conf').with_ensure('installed') }
      it { should contain_file('snmpd.sysconfig').with_ensure('installed') }
      it { should contain_service('snmpd').with_ensure('running') }
      it { should contain_file('snmptrapd.conf').with_ensure('installed') }
      it { should contain_file('snmptrapd.sysconfig').with_ensure('installed') }
      it { should contain_service('snmptrapd').with_ensure('stopped') }
    end
